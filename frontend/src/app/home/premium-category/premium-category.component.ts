...
@Component({
  selector: 'app-premium-category',
  templateUrl: './premium-category.component.html',
  styleUrls: ['./premium-category.component.css']
})
export class PremiumCategoryComponent implements OnInit {
  ...
  constructor(private http: HttpService, private router: Router, private auth: AuthService) {
    if(this.auth.isPremiumUser() == 'false'){
  ...
  ngOnInit(): void {
    ...
    // get category data
    this.http.get('/category/getAllPremiumCategories').subscribe(
      (res => {
